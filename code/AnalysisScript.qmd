...
library(vegan)
library(readxl)
library(microViz) #for the NMDS hull creation
library(lubridate)
...
h <- read.csv("data/environmental_covariates.csv") #environmental covariates (temperature, tide, etc)
...
COI_p <- COI_m %>% 
  ...
  mutate(Date.and.Time = datetime) %>% 
  left_join(h %>% mutate(Date.and.Time = as_datetime(Date.and.Time)))
...
## COI Annotation curation
COI_05 <- COI_04 %>%
  rownames_to_column('tube_no') %>%
  pivot_longer(cols=-tube_no, names_to = "BestTaxon", values_to = "Reads") %>%
  # Different species reported in the area (same genus)
  mutate(BestTaxon = if_else(BestTaxon == "Acrochaetium alariae", "Acrochaetium secundatum", BestTaxon)) %>%
  # Two different sp reported, call back to genus
  mutate(BestTaxon = if_else(BestTaxon == "Bougainvillia superciliaris", "Bougainvillia", BestTaxon)) %>%
  # Up to spp in this area
  mutate(BestTaxon = if_else(BestTaxon == "Calycella", "Calycella syringa", BestTaxon)) %>%
  # Change to genus present in the area
  mutate(BestTaxon = if_else(BestTaxon == "Obeliidae", "Obelia", BestTaxon)) %>%
  # Change to genus present in the area
  mutate(BestTaxon = if_else(BestTaxon == "Cocconeiopsis kantsiensis", "Cocconeiopsis", BestTaxon)) %>%
  # Change to genus present in the area
  mutate(BestTaxon = if_else(BestTaxon == "Kurtiella bidentata", "Bivalvia", BestTaxon)) %>%
  # rename to spp present in the area:
  mutate(BestTaxon = if_else(BestTaxon == "Nuttallia ezonis", "Nuttallia obscurata", BestTaxon)) %>%
  # call back to genus:
  mutate(BestTaxon = if_else(BestTaxon == "Grammonema striatula", "Grammonema", BestTaxon)) %>%
  # Change to genus present in the area
  mutate(BestTaxon = if_else(BestTaxon == "Kappaphycus", "Gigartinales", BestTaxon)) %>%
  # call back to genus:
  mutate(BestTaxon = if_else(BestTaxon == "Mycale cecilia", "Mycale", BestTaxon)) %>%
  # call back to genus:
  mutate(BestTaxon = if_else(BestTaxon == "Nannochloropsis granulata", "Nannochloropsis", BestTaxon)) %>%
  # call back to genus:
  mutate(BestTaxon = if_else(BestTaxon == "Phaeocystis globosa", "Phaeocystis", BestTaxon)) %>%
  # call back to genus:
  mutate(BestTaxon = if_else(BestTaxon == "Haslea avium", "Haslea", BestTaxon)) %>%
  # call back to genus:
  mutate(BestTaxon = if_else(BestTaxon == "Skeletonema japonicum", "Skeletonema", BestTaxon)) %>%
  # call back to genus:
  mutate(BestTaxon = if_else(BestTaxon == "Yihiella yeosuensis", "Yihiella", BestTaxon)) %>%
  # call back to genus:
  mutate(BestTaxon = if_else(BestTaxon == "Turritopsis lata", "Turritopsis", BestTaxon)) %>%
  # Change to genus present in the area
  mutate(BestTaxon = if_else(BestTaxon == "Vimineoleathesia japonica", "Chordariaceae", BestTaxon)) %>%
  # Change to genus present in the area
  mutate(BestTaxon = if_else(BestTaxon == "[Olpidiopsis] porphyrae var. scotiae (nom. inval.)", "Chordariaceae", BestTaxon)) %>%
  # rename to spp present in the area:
  mutate(BestTaxon = if_else(BestTaxon == "Stauridiosarsia producta", "Stauridiosarsia cliffordi", BestTaxon)) %>%
  # call back to genus:
  mutate(BestTaxon = if_else(BestTaxon == "Thalassiosira punctigera", "Thalassiosira", BestTaxon)) %>%
  group_by(tube_no, BestTaxon) %>%
  summarise(Reads = sum(Reads),.groups = "drop") %>%
  pivot_wider(names_from = BestTaxon, values_from = Reads) %>%
    mutate(tube_no=as.numeric(tube_no)) %>%
    arrange(tube_no) %>%
  column_to_rownames("tube_no")
...
rownames(COI_05) == COI_p$Tube
rownames(MV1_05) == MV1_p$Tube
rownames(MFU_05) == MFU_p$Tube
```
...
##COI
    ...
    #make common proportion dataset  
    COI_common_prop <- COI_05 %>% 
      dplyr::select(which(colSums(.) > 5000)) %>%  # threshold for plotting
      rownames_to_column("Tube") %>% 
      pivot_longer(-Tube) %>% 
      group_by(Tube) %>% 
      mutate(value = value/sum(value)) %>% 
...
### COI 
  ...
  dist_coi <- COI_idx %>% 
    t() %>% 
    vegdist(method = "bray", binary = T)
  
  MDS_coi <- dist_coi %>% 
    as.matrix() %>% 
    metaMDS(trymax = 1000)
  
  #Look at autoregression
  COI_p %>%
  mutate(day_idx = match(COI_p$Date, unique(COI_p$Date))) %>% 
  mutate(half_idx = ifelse(Date < "2025-07-30", "A", "B")) %>% 
  mutate(twoday_idx = COI_p$Date %>% cut(4)) %>%
  bind_cols(MDS_COI$points) %>%
  # filter(half_idx == "A") %>%
  # filter(Day.Night == "night") %>% 
  ggplot(aes(x = MDS1, y = MDS2, color = as.factor(half_idx))) +
  geom_point() +
  stat_chull(aes(fill = interaction(Day.Night, Treatment)), alpha = 0.3) +
  geom_label(aes(x = MDS1, y = MDS2, label = as.factor(day_idx))) +
  theme_minimal()

# permanova
res <- adonis2(dist_COI ~ half_idx + Day.Night + Treatment%in%Day.Night, data = COI_p %>%
          mutate(half_idx = ifelse(Date < "2025-07-30", "A", "B")), by = "terms")
...
```{r indicator_species_analysis}

grp <- with(COI_p, interaction(Day.Night,Treatment), drop = TRUE)
ind <- COI_idx %>% t() %>%
	multipatt(grp_4, func = "IndVal.g", restcomb = c(1,2,3,4), control = how(nperm = 999))

summary(ind)

# Filter list by p-value
ind_result <- ind$sign

ind_result <- ind_result %>%
	mutate(species = rownames(ind_result))
...
# Pivot longer the COI_idx
COI_idx_long <- COI_idx %>%
	rownames_to_column("species") %>%
	pivot_longer(cols = -species,
							 names_to = "Tube",
							 values_to = "eDNA_index") %>%
	mutate(Tube=as.numeric(Tube)) %>%
	left_join(COI_p, by="Tube") %>%
	left_join(COI_01 %>% select("BestTaxon", "Class"), by=c("species"="BestTaxon")) %>%
	mutate(Treatment_int=paste0('s.',Day.Night)) %>%
	left_join(.,ind_result,by=c('species')) %>%
	filter(!is.na(index))


COI_idx_long %>% mutate(Day.Night=factor(Day.Night,levels = c('day','night'))) %>% 
	filter(index%in%c(1)) %>%
	ggline(., x='Day.Night', y='eDNA_index',color = 'species',add = c('mean_se','jitter'),
				 facet.by = c("Class"))+
	theme(legend.position = "right") 

COI_idx_long %>% mutate(Day.Night=factor(Day.Night,levels = c('day','night'))) %>% 
	filter(index%in%c(2,4)) %>%
	filter(Day.Night=='night') %>% 
	#filter(Class == "Polychaeta") %>%
	ggline(., x='Treatment', y='eDNA_index',color = 'species',add = c('mean_se','jitter'),
				 facet.by = c("Class"))+
	theme(legend.position = "right") 
...
#TODO straighten plotting details for shading and labeling, etc
...
COI_common_prop %>% 
  ...
  ggplot(aes(x = Tube, y = value, fill = name)) +
  # Add alternating day/night shading rectangles first
  geom_rect(data = data.frame(
    xmin = seq(0.5, by = 12, length.out = ceiling(max(as.numeric(COI_p$Tube))/12)),
    xmax = seq(6.5, by = 12, length.out = ceiling(max(as.numeric(COI_p$Tube))/12)),
    ymin = -Inf, ymax = -0.05
  ), aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), 
  fill = "lightblue", alpha = 0.3, inherit.aes = FALSE) +
  
  geom_rect(data = data.frame(
    xmin = seq(6.5, by = 12, length.out = ceiling(max(as.numeric(COI_p$Tube))/12)),
    xmax = seq(12.5, by = 12, length.out = ceiling(max(as.numeric(COI_p$Tube))/12)),
    ymin = -Inf, ymax = -0.05
  ), aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), 
  fill = "darkblue", alpha = 0.3, inherit.aes = FALSE) +
  
  geom_col() + xlab("") + ylab("Proportion") +
  scale_x_discrete(labels = unique(COI_p$Date), 
                   breaks = c(1:max(COI_p$Tube))[match(unique(COI_p$Date), COI_p$Date)]) +
  theme(
